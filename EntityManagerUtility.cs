...
namespace E7.EcsGadgets
{
    ...
    public class EntityManagerUtility
    {
        ...
        ///
        /// This method allocates a List and throw it away, generating garbage.
        /// </summary>
        /// <returns>-1 when not found. That means this SCD value has not been used anywhere yet or not used anymore.</returns>
        public int IndexOfSharedComponentData<T>(T sharedComponentValue) where T : struct, ISharedComponentData
        {
            List<T> searchList = new List<T>();
            em.GetAllUniqueSharedComponentData(searchList);
            return searchList.IndexOf(sharedComponentValue);
        }
    }
}